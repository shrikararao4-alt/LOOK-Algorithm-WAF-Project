...
export function computeFCFS(queue, head) {
...
export function computeSSTF(queue, head) {
...
export function computeSCAN(queue, head, direction, diskMax = DISK_MAX) {
...
export function computeCSCAN(queue, head, direction, diskMax = DISK_MAX) {
  const sorted = [...new Set(queue)].sort((a, b) => a - b);
...
export function computeLOOK(queue, head, direction) {
  const sorted = [...new Set(queue)].sort((a, b) => a - b);
  const left = sorted.filter((t) => t < head).reverse();
  const right = sorted.filter((t) => t >= head);
...
export function computeCLOOK(queue, head, direction) {
  const sorted = [...new Set(queue)].sort((a, b) => a - b);
  const left = sorted.filter((t) => t < head);
  const right = sorted.filter((t) => t >= head);
